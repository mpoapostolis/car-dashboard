...
socket.on("update-speed",function(speed){
  invokeCommand(`s ${speed}`)
})

socket.on("change-lane",function(offset){
  invokeCommand(`c ${offset}`)
})


config.read(process.argv[2], function(carId, startlane, mqttClient, name) {
  console.log(startlane);
                ...
                function(callback) {
                  if (characteristic.uuid == 'be15bee06186407e83810bd89c4d8df4') {
                    ...
                    characteristic.on('read', function(data, isNotification) {
                      receivedMessages.handle(data, mqttClient, name);
                    });
                  }
